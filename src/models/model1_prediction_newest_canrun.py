...
if __name__ == "__main__":
    ...
    season_type = get_scoring_system(CURRENT_SEASON)
    ensemble = ResidualCalibrationEnsemble()

    print(f"Season={CURRENT_SEASON}, Type={season_type}")
    ...
    for week in range(1, MAX_WEEKS + 1):
        ...
        if int(np.sum(eliminated_flag)) == 0:
            print(f"\nWeek={week}: no elimination label found, skip")
            continue
        ...
        eliminated_pred_idx = int(np.argmin(fan_score))
        metric_name = "Fan_Score"

        if season_type == "rank" and CURRENT_SEASON >= 28 and len(names) >= 2:
            bottom2_indices = np.argsort(fan_score)[:2].tolist()
            ...
            if revote_elim is not None:
                ...
                metric_name = "Fan_Score_Bottom2_Revote"
        ...
        print(result_df.sort_values(metric_name, ascending=True).head(5))
