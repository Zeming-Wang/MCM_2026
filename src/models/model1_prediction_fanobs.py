...
# ==========================================
# 2. Model B：粉丝投票反演模型（使用淘汰作为观测）
# ==========================================
def build_fan_vote_model(industry_idx, age_z, eliminated_flag):
    with pm.Model() as model:
        ...
        pm.Deterministic("fan_percent", pm.math.softmax(fan_latent))
...
if __name__ == "__main__":
    ...
    DATA_PATH = r"d:\MCM_2026_O\data\processed\processed_mcm_weekly_long.csv"
    ...
    if os.path.exists(DATA_PATH):
        ...
        for week in range(1, MAX_WEEKS + 1):
            ...
            if int(np.sum(eliminated_flag)) == 0:
                p_fan = np.ones(len(week_df)) / len(week_df)
            else:
                fan_model = build_fan_vote_model(ind, age_z, eliminated_flag)
                with fan_model:
                    trace = pm.sample(
                        DRAWS,
                        tune=TUNE,
